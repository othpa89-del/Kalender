...
//  Nur Überschrift + Notizfeld. Kein Datum, keine Priorität. Cloud-synchron.
// ===========================================================================
...
export function NiceToKnow({ t, items, setItems }) {
  ...
  function reset() { setTitle(""); setText(""); setEditId(null); }
  function save() {
    if (!title.trim() && !text.trim()) return;
    if (editId) {
      setItems(items.map((x) => (x.id === editId ? { ...x, title: title.trim(), text: text.trim() } : x)));
    } else {
      setItems([{ id: uid("note"), title: title.trim(), text: text.trim(), createdAt: Date.now() }, ...items]);
    }
    reset();
  }
  function edit(x) { setTitle(x.title || ""); setText(x.text || ""); setEditId(x.id); window.scrollTo({ top: 0, behavior: "smooth" }); }
